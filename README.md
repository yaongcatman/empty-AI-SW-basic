...
1. `sudo ufw status verbose`
   ...
   * **어떻게/왜**: 현재 어떤 문이 열려 있는지 상세히 파악하여 중복 설정을 방지합니다.

2. `sudo apt update && sudo apt install ufw -y`
   * **언제/어디서**: 서버에 방화벽 프로그램(UFW)이 없을 때 설치하기 위해 사용합니다.
   * **어떻게/왜**: 최신 패키지 목록을 불러오고, 자동으로 '예(yes)'를 선택하여 UFW를 설치합니다.

3. `sudo ufw allow 20022/tcp`
   * **언제/어디서**: SSH 포트를 20022로 변경한 후, 이 문을 통과시켜야 할 때 사용합니다.
   * **어떻게/왜**: 20022번 통로를 TCP 프로토콜(안전한 연결 방식)로 열어 접속을 허용합니다.

4. `sudo ufw allow 15034/tcp`
   * **언제/어디서**: 특정 애플리케이션이나 서비스가 15034 포트를 사용할 때 실행합니다.
   * **어떻게/왜**: 서비스 운영을 위해 해당 포트로 들어오는 신호를 차단하지 않도록 설정합니다.

5. `sudo ufw --force enable`
   * **언제/어디서**: 모든 포트 설정을 마친 후, 방화벽을 실제 가동할 때 사용합니다.
   * **어떻게/왜**: 확인 질문 없이 즉시 방화벽을 활성화하여 설정을 적용합니다.

6. `sudo ufw status`
   * **언제/어디서**: 모든 작업이 끝난 후 최종적으로 사용합니다.
   * **어떻게/왜**: 내가 열어준 포트들이 `ALLOW` 상태로 잘 목록에 있는지 확인합니다.
